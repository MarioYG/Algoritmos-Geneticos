...
class Chromosome:
    ...
    def crossover(self, other): ##Funcion crossover 1 (por defecto)
    ...
    def crossover_2(self, other): ##Funcion crossover 2
        punto_cruce = randint(1, len(other.data)-3)
        hijo1 = self.data[:punto_cruce]
        j = -1
        for i in other.data :
            j += 1
            if (i not in hijo1) :
...
class GA:
    ...
    def get_best(self): ##Funcion selectora 1 (solo funciona en Python 2)
        return [p for (a, p) in sorted(zip(self.adaptability(), self.population))][0]
    
    def get_best_2(self) : ##Funcion selectora 2 (por defecto)
